...
//    private final TransactionTemplate transactionTemplate;
  ...
  public Optional<UserAggregate> findById(String id) {
    ...
    if (!CollectionUtils.isEmpty(wishList)) {
      userAgg.addWishlists(
          wishList.stream().map(WishItemEntity::toWishItem).collect(Collectors.toList()));
    }
  ...
  public Long save(UserAggregate userAggregate) {
    ...
    if (!CollectionUtils.isEmpty(userAggregate.getWishlists())) {
      List<WishItemEntity> wishlist = userAggregate.getWishlists().stream()
          .peek(e -> e.setUserId(user.getId())).map(WishItemEntity::fromUserAggregate)
  ...
  public Long update(UserAggregate userAggregate) {
    ...
    if (!CollectionUtils.isEmpty(userAggregate.getWishlists())) {
      List<WishItemEntity> wishlist = userAggregate.getWishlists().stream()
          .peek(e -> e.setUserId(user.getId())).map(WishItemEntity::fromUserAggregate)
          .collect(Collectors.toList());
      List<WishItemEntity> toBeAdded = wishlist.stream().filter(e -> e.getId() == null)
          .collect(Collectors.toList());
  ...
  public List<UserAggregate> findAll() {
    ...
    Map<Long, List<UserAggregate>> mapper = userList.stream().map(UserEntity::toUserAggregate)
        .collect(Collectors.groupingBy(e -> Long.parseLong(e.getId())));
    List<WishItemEntity> wishList = wishItemRepository.findAllByUserIdIn(
        new ArrayList<>(mapper.keySet()));
    if (!CollectionUtils.isEmpty(wishList)) {
      Map<String, List<WishItemEntity>> wishlistMapper = wishList.stream()
      ...
      return mapper.values().stream().flatMap(e -> e.stream()).peek(e -> {
            if (wishlistMapper.get(e.getId()) != null) {
              e.addWishlists(wishlistMapper.get(e.getId()).stream().map(WishItemEntity::toWishItem)
                  .collect(Collectors.toList()));
            }
          }
      ).collect(Collectors.toList());
