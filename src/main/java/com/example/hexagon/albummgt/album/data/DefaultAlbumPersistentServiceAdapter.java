...
import com.example.hexagon.albummgt.album.exception.DomainAlbumException;
...
public class DefaultAlbumPersistentServiceAdapter implements AlbumPersistentService {
...
//    private final TransactionTemplate transactionTemplate;
    ...
    public List<AlbumAggregate> findAll() {
        ...
        Map<Long, List<AlbumAggregate>> mapper = albumList.stream().map(AlbumEntity::toAlbumAggregate).collect(Collectors.groupingBy(e -> Long.parseLong(e.getId())));
        List<ArtistEntity> artistList = artistRepository.findAllByAlbumIdIn(new ArrayList<>(mapper.keySet()));
        if (!CollectionUtils.isEmpty(artistList)) {
            Map<String, List<ArtistEntity>> albumListMapper = artistList.stream().collect(Collectors.groupingBy(e -> e.getAlbumId() + ""));
            return mapper.values().stream()
                    .flatMap(e -> e.stream())
                    .peek(e -> e.setArtist(albumListMapper.get(e.getId()).stream().map(ArtistEntity::toArtist).findFirst().orElse(null)))
